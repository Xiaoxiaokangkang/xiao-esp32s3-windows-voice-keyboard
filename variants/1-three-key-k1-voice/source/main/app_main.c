...
static bool send_keyboard_modifiers(uint8_t modifiers)
{
...
static void button_task(void *argument)
{
    ...
    while (true) {
        const bool sample_pressed = gpio_get_level(BUTTON_K1_GPIO) == 1;
        ...
        if (sample_pressed != stable_pressed &&
            now - changed_at >= pdMS_TO_TICKS(20)) {
            stable_pressed = sample_pressed;
            gpio_set_level(USER_LED_GPIO, stable_pressed ? 0 : 1);
            hotkey_state = stable_pressed ? HOTKEY_PRESS_CTRL
                                          : HOTKEY_RELEASE_GUI;
        }

        if (!tud_mounted()) {
            hotkey_state = stable_pressed ? HOTKEY_PRESS_CTRL : HOTKEY_IDLE;
        } else {
            switch (hotkey_state) {
            case HOTKEY_PRESS_CTRL:
                if (send_keyboard_modifiers(KEYBOARD_MODIFIER_LEFTCTRL)) {
                    hotkey_state = HOTKEY_PRESS_BOTH;
                }
                break;
            case HOTKEY_PRESS_BOTH:
                if (send_keyboard_modifiers(HOTKEY_MODIFIERS)) {
