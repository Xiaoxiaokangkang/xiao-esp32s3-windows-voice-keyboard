...
typedef enum {
    ...
    ACTION_ENTER_DOWN,
    ACTION_ENTER_UP,
    ACTION_SELECT_ALL_DOWN,
    ACTION_SELECT_ALL_UP,
    ACTION_BACKSPACE_DOWN,
    ACTION_BACKSPACE_UP,
} keyboard_action_t;
...
static void button_task(void *argument)
{
    ...
    TickType_t k3_pressed_at = 0;
    bool k3_action_sent = false;
    ...
    while (true) {
        ...
        if (k1_edge != 0) {
            gpio_set_level(USER_LED_GPIO, k1.stable_pressed ? 0 : 1);
            hotkey_state = k1.stable_pressed ? HOTKEY_PRESS_CTRL
                                             : HOTKEY_RELEASE_GUI;
        }
        if (k2_edge > 0 && hotkey_state == HOTKEY_IDLE && action == ACTION_NONE) {
            action = ACTION_ENTER_DOWN;
        }
        if (k3_edge > 0) {
            k3_pressed_at = now;
            k3_action_sent = false;
        } else if (k3_edge < 0) {
            k3_action_sent = false;
        }
        if (k3.stable_pressed && !k3_action_sent &&
            now - k3_pressed_at >= pdMS_TO_TICKS(1500) &&
            hotkey_state == HOTKEY_IDLE && action == ACTION_NONE) {
            k3_action_sent = true;
            action = ACTION_SELECT_ALL_DOWN;
        }

        if (!tud_mounted()) {
            hotkey_state = k1.stable_pressed ? HOTKEY_PRESS_CTRL : HOTKEY_IDLE;
            action = ACTION_NONE;
            ...
            case ACTION_ENTER_UP:
                ...
                break;
            case ACTION_SELECT_ALL_DOWN:
                if (send_keyboard_report(KEYBOARD_MODIFIER_LEFTCTRL, HID_KEY_A))
                    action = ACTION_SELECT_ALL_UP;
                break;
            case ACTION_SELECT_ALL_UP:
                if (send_keyboard_report(0, 0)) action = ACTION_BACKSPACE_DOWN;
                break;
            case ACTION_BACKSPACE_DOWN:
                if (send_keyboard_report(0, HID_KEY_BACKSPACE))
                    action = ACTION_BACKSPACE_UP;
                break;
            case ACTION_BACKSPACE_UP:
                if (send_keyboard_report(0, 0)) action = ACTION_NONE;
                break;
            case ACTION_NONE:
                break;
